...
const NavigationButtons: React.FC<NavigationButtonsProps> = ({
  ...
  return (
    <View className="mb-1 flex-row justify-between space-x-2">
      {/* Back Button */}
      <TouchableOpacity
        onPress={onPrevious}
        disabled={step === 1}
        className={`mr-2 flex-1 flex-row items-center justify-between rounded border-2 px-4 py-3 ${
      ...
      {step < totalSteps ? (
        <TouchableOpacity
          onPress={onNext}
          disabled={!canProceed}
          className={`w-1/2 flex-row items-center justify-between rounded border-2 px-4 py-1 ${
